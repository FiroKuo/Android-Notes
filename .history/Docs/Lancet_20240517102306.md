...
```
`ClassifiedContentProvider`是一个组合对象，它包含从`Jar`和`Dir`中读取 `classes` 的方式，分别对应`JarContentProvider`和`DirectoryContentProvider`。`QualifiedContentTask`的`call`方法最终触发`PreAnalysisClassFetcher`的