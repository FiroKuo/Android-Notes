...
`getService`返回后通会将 IBinder 转为业务逻辑接口，比如使用AIDL时：
```java
...
- Proxy 中持有了 BinderProxy 的实例，即 `mRemote`，当发起远程调用的时候，调用关系即: `XxxProxy->BinderProxy->BpBinder->BBinder->Binder`，这样，就从 client 到 server，从 Java 层到 native 层，完成了一次调用。
