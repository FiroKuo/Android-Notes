...
incremental = preClassAnalysis.execute(incremental, context);
```
这几行代码可以看作预分析的流程：`TransformContext`初始化的时候准备了`class`的来源，如`Collection<JarInput> allJars;`、`Collection<DirectoryInput> allDirs;`；`incremental`变量代表是否是增量编译，当 hook 类所在的来源文件发生改变时，必须全量编译。