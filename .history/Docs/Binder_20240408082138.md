...
## ServiceManager
它是 Binder 大管家，类似于 DNS 服务器，本身也是一个 Binder Server。它是 init 进程负责启动的，具体是在解析 init.rc 配置文件时启动的，Android7.0 后对 init.rc 配置文件进行了拆分，ServiceManager 启动的脚本在 ServiceManager.rc 中。
