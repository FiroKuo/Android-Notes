...
```
`ContextReader`提供了一种线程池中读取 `classes` 的环境，它的`accept`方法是阻塞的:
```
...
```
`ClassifiedContentProvider`是一个组合对象，它包含从`Jar`和`Dir`中读取 `classes` 的方式，分别对应`JarContentProvider`和`DirectoryContentProvider`。`QualifiedContentTask`的`call`方法