...
```
这里的流程会走 else 分支，`saveData`最终将解析后的`classes`信息缓存到`build/lancet/lancet/buildCache.json`中，`fullyParse`将进行`classes`的读取与解析：


`ContextReader`提供了一种线程池中读取 class 的环境，它的`accept`方法是阻塞的:
```
