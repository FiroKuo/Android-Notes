...
当从 ServiceManager 中查询到新的 Server，它对应的是 BBinder，它的内存引用会被 Binder 驱动映射为`handle`值（跨进程，同进程可以直接返回 BBinder 的内存地址）返回到 Client，这个时候会创建 BpBinder(handle)，以及 Java 层的 BinderProxy，并为具体的 XxxServiceProxy 的`mRemote`赋值。
